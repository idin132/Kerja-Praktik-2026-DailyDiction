...
// Pastikan nama fungsi API untuk ngambil detail review kamu bener, misal: getReviewBySlug
...
export default async function DetailReview({
  ...
  getGameReviewBySlug(slug), // <-- Ganti di sini
          ...
          <div className="lg:col-span-8">
            <article>
              ...
              <div className="mb-8 space-y-6">
                ...
                {/* Badge Platform & Rating */}
                <div className="flex flex-wrap items-center gap-3">
                  {review.platform && (
                    <span className="flex items-center gap-1.5 rounded bg-brand-cyan/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-brand-cyan border border-brand-cyan/30">
                      <Gamepad2 className="h-3.5 w-3.5" />
                      {review.platform}
                    </span>
                  )}
                  {review.rating && (
                    <span className="flex items-center gap-1.5 rounded bg-yellow-500/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-yellow-500 border border-yellow-500/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-dark-border pt-8 mt-8">
              {/* Previous Review */}
              {prevReview ? (
                // Pastikan link-nya mengarah ke /review/, bukan /artikel/
