...
import { getGameReviews, formatImageUrl, ArticleItem } from "@/lib/api";
...
export default async function ReviewPage() {
  const reviews: ArticleItem[] = await getGameReviews();
  ...
  return (
    <div className="min-h-screen bg-dark-bg text-text-primary selection:bg-brand-crimson selection:text-white">
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-12 sm:px-6 lg:px-8">
        ...
        {featuredReview && (
          <div className="mb-12">
            <Link href={`/review/${featuredReview.slug}`} prefetch={false} className="group block overflow-hidden rounded-2xl border border-dark-border bg-dark-card transition-colors hover:border-brand-cyan/50">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5">
                <div className="relative aspect-video md:aspect-auto lg:col-span-3 overflow-hidden">
                  <img
                    src={formatImageUrl(featuredReview.image_url || featuredReview.image_full_url || featuredReview.thumbnail_url || featuredReview.thumbnail)}
                ...
                <div className="flex flex-col justify-center p-6 md:p-8 lg:col-span-2 lg:p-12">
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    {parsePlatforms(featuredReview.platform).map((plat, idx) => (
                      <span key={idx} className="rounded bg-brand-cyan/20 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-brand-cyan border border-brand-cyan/30">
                        {plat}
                      </span>
                    ))}
                  </div>
        ...
        {reviews.length > 0 ? (
              ...
              return (
                <Link key={review.id} href={`/review/${review.slug}`} className="group flex flex-col overflow-hidden rounded-xl border border-dark-border bg-dark-card transition-colors hover:border-brand-cyan/50 hover:shadow-lg">
                  <div className="relative aspect-[4/3] w-full overflow-hidden border-b border-dark-border/50">
                    <img
                      src={formatImageUrl(review.image_url || review.image_full_url || review.thumbnail_url || review.thumbnail)}
                      alt={review.title}
          ...
          <div className="rounded-2xl border border-dark-border bg-dark-card p-12 text-center text-text-muted">
            Belum ada review game yang dipublikasikan.
          </div>
        )}
      </main>
      <Footer />
