...
export default async function DetailArtikel({
  ...
  if (Array.isArray(article.category)) {
    ...
    try {
      categoryList = article.category.startsWith('[') ? JSON.parse(article.category) : [article.category];
    } catch {
      categoryList = [article.category];
    }
  }
            ...
            <div className="lg:col-span-8">
              <article>
                ...
                <div className="mb-8 space-y-6">
                  ...
                  {/* Badge Kategori Dinamis (Multiple Tags) */}
                  <div className="flex flex-wrap items-center gap-2">
                    {categoryList.length > 0 ? (
                      categoryList.map((cat, idx) => (
                        <span key={idx} className="rounded bg-brand-cyan/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-brand-cyan border border-brand-cyan/30">
                          {cat}
                        </span>
                      ))
                    ) : (
