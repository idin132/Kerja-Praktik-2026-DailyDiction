...
export default function Navbar() {
  ...
  // FUNGSI SEARCH (Dari Rizqi)
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (keyword.trim()) {
      router.push(`/search?q=${keyword}`);
      setIsOpen(false); // Tutup menu mobile otomatis kalau lagi buka di HP
    }
  };
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-dark-border bg-dark-bg/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop Navigation Links (Dari Idin) */}
        ...
        <div className="flex items-center gap-3">
          ...
          {/* DESKTOP SEARCH FORM (Dari Rizqi) */}
      ...
      {/* Mobile Drawer Navigation Menu Overlay (Dari Idin) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            <div className="space-y-2 px-4 pt-3 pb-6 font-mono text-sm">
              ...
              {/* MOBILE SEARCH FORM (Adaptasi buatan Rizqi buat di HP) */}
              <form onSubmit={handleSearch} className="flex items-center bg-dark-bg border border-dark-border rounded-xl px-4 py-3 mb-4 focus-within:border-brand-cyan transition-colors">
