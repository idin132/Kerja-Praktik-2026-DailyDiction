...
export default function YoutubeHero({ videos = [] }: { videos?: any[] }) {
  ...
  if (!heroVideos || heroVideos.length === 0) {
    return (
      <div className="rounded-2xl border border-dark-border bg-dark-card p-8 text-center text-xs font-mono text-text-muted">
        Video sedang tidak dapat dimuat atau limit API tercapai.
      </div>
    );
  }
